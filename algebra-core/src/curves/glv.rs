...
pub trait GLVParameters: Send + Sync + 'static + ModelParameters {
    ...
    fn glv_scalar_decomposition_inner(
        ...
        let mut k2_field = if Self::B1_IS_NEG {
            Self::ScalarField::from_repr_unchecked(d2)
        } else {
            Self::ScalarField::from_repr_unchecked(d1)
        };
        if Self::B1_IS_NEG {
            k2_field -= &Self::ScalarField::from_repr_unchecked(d1);
        } else {
            k2_field -= &Self::ScalarField::from_repr_unchecked(d2);
        }
